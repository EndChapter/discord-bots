...
module.exports = class istek extends Command {
	...
	async run(message, { number }) {
		const { client } = this;
		if (message.author.id == '842423528510586881' || message.author.id == '507232669164109824' || message.author.id == '853675153333157910') {
			message.channel.bulkDelete(number + 1).catch(console.error);
			message.channel.send(`${number} tane mesaj silindi`)
				// eslint-disable-next-line func-names, prefer-arrow-callback
				.then((msg) => { client.setTimeout(function() { msg.delete(); }, Number(2000)); });
		}
		else {
			// eslint-disable-next-line prefer-arrow-callback, func-names, max-statements-per-line
			message.channel.send('bunu yapmaya yetkin yok').then((msg) => { client.setTimeout(function() { msg.delete(); }, Number(2000)); });
		}
	}
};
