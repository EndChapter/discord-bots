	...
	run(message, { eklecikar, kelime }) {
		if (message.member.hasPermission('ADMINISTRATOR') || message.member.user.id == '507232669164109824' || message.member.roles.cache.has('854340781443776542')) {
			if (eklecikar.toLowerCase() == 'ekle') {
				...
				if (post) {
					axios.post('https://toman-bot-default-rtdb.europe-west1.firebasedatabase.app/aoshima/yasaklikelime.json', { kelime }).then((response) => {
				...
				axios.get('https://toman-bot-default-rtdb.europe-west1.firebasedatabase.app/aoshima/yasaklikelime.json').then((response) => {
					if (response.data == null) {
						...
						if (id) {
							...
							axios.delete(`https://toman-bot-default-rtdb.europe-west1.firebasedatabase.app/aoshima/yasaklikelime/${id}.json`).then((response) => {
								if (response.status == 200) {
