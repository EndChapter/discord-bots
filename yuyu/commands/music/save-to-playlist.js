...
module.exports = class SaveToPlaylistCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      guildOnly: true,
      description: 'Bir şarkı veya playlisti seçtiğiniz bir playliste kaydeder',
      args: [
        {
          key: 'playlist',
          prompt: 'Kaydetmek istediğin playlist nedir?',
          type: 'string'
        },
        {
          key: 'url',
          prompt:
            'Hangi url\'i playlistine eklemek istersin? Bu aynı zamanda bir playlist Url\'i olabilir',
          type: 'string',
  ...
  async run(message, { playlist, url }) {
    ...
    if (!dbUserFetch) {
      message.reply('Sıfır kaydedilmiş playlistin var!');
      return;
    }
    ...
    if (found) {
      ...
      if (Array.isArray(processedURL)) {
        ...
        message.reply('Karar verilen playlist başarı ile kaydedildi!');
      } else {
        ...
        message.reply(
          ...
          }** müziklerini **${playlist}** playlistine kaydettim`
        );
      }
      db.set(message.member.id, { savedPlaylists: savedPlaylistsClone });
    } else {
      message.reply(`Şu isimle playlistin yok: ${playlist}`);
      return;
    }
  }
  ...
  static async processURL(url, message) {
    if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
      const playlist = await youtube.getPlaylist(url).catch(function() {
        message.reply(':x: Playlist ya özel ya da mevcut değil!');
      });
      if (!playlist) {
        return false;
      }
      const videosArr = await playlist.getVideos().catch(function() {
        message.reply(
          ':x: Playlistteki videolardan birini alırken sorun oluştu!'
        );
        return;
      });
      let urlsArr = [];
    ...
    const video = await youtube.getVideoByID(id).catch(function() {
      message.reply(':x: Sağladığınız videoyu alırken bir sorun oluştu!');
      return;
    });
    if (video.raw.snippet.liveBroadcastContent === 'live') {
      message.reply("Canlı yayınları desteklemiyorum!");
      return false;
    }
