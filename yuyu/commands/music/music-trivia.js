...
module.exports = class MusicTriviaCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      group: 'music',
      description: 'Arkadaşlarınla müzik quizi çözerek eğlen!',
      guildOnly: true,
      ...
      args: [
        {
          key: 'numberOfSongs',
          prompt: 'Kaç şarkı ile quiz yapmak istersin?',
          type: 'integer',
          min: 1,
          //default: 5,
          max: 15
  ...
  async run(message, { numberOfSongs }) {
    ...
    if (!voiceChannel) {
      message.reply(':no_entry: Lütfen bir ses kanalına katıl ve tekrar dene!');
      return;
    }
    if (message.guild.musicData.isPlaying === true)
      return message.channel.send(':x: Bir quiz veya bir şarkı çalma işlemi zaten çalışıyor!');
    ...
    const infoEmbed = new MessageEmbed()
      ...
      .setDescription(
        `:notes: Hazır ol! ${numberOfSongs} şarkı var, Şarkıcı veya şarkı ismini bulman için şarkı başına 30 saniyen var. İyi Şanslar!
    ...
    queue[0].voiceChannel.join().then(function(connection) {
      const dispatcher = connection
        ...
        .on('error', async function(e) {
          message.reply(':x: Bu şarkıyı oynatırken bir hata oluştu!');
          console.log(e);
          if (queue.length > 1) {
            queue.shift();
